...
namespace HoloToolkit.Unity.InputModule
{
    ...
    {
        public MeshRenderer cursorMeshRenderer;
        ...
        public void StartDragging()
        {
            ...
            if (isDragging)
            {
                //Debug.Log("isDragging==trueg");
                return;
            }
            //Debug.Log("StartDragingGO: " + transform.name);
            if (transform.name.Equals("1"))
            { // Rony's Kot.
                GetComponent<MeshRenderer>().enabled = false; // No render = invisible.
                transform.Find("Cylinder").GetComponent<MeshRenderer>().enabled = false;
                // Hide cursor.
                if (cursorMeshRenderer != null) { cursorMeshRenderer.enabled = false; }
              /*  GameObject.Find("cursor_interaction").SetActive(false);
                GameObject.Find("cursor_observation").SetActive(false);
                GameObject.Find("cursor_press").SetActive(false); */
              // GameObject.Find("Mastoid Canal 14.8.17").GetComponent<MeshCollider>().enabled = true; // This takes about 5 seconds. WAY too slow.
        ...
        public void StopDragging()
        {
            ...
            if (transform.name.Equals("1"))
            {
                ...
                if (cursorMeshRenderer != null) { cursorMeshRenderer.enabled = true; }
                /*   GameObject.Find("Cursor_v1").transform.Find("cursor_interaction").gameObject.SetActive(true);
                   GameObject.Find("Cursor_v1").transform.Find("cursor_observation").gameObject.SetActive(true);
                   GameObject.Find("Cursor_v1").transform.Find("cursor_press").gameObject.SetActive(true); */
                //GameObject.Find("Mastoid Canal 14.8.17").GetComponent<MeshCollider>().enabled = false;
